...
const MARKET_VENDOR_FILES = ['operatingRatio.ts'];
const MARKET_CORE_DIR = path.join(REPO_ROOT, 'packages', 'core', 'src', 'market');
...
function main() {
  ...
  for (const name of MARKET_VENDOR_FILES) {
    written.push(syncFile(name, { fromDir: MARKET_CORE_DIR }));
  }
