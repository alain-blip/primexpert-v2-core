...
import {
  ...
  computeNetOperatingIncomeVarianceRatio,
  DSCR_RULES,
  formatNetOperatingIncomeVariancePct,
...
export function FinancabiliteTab({ residence }: FinancabiliteTabProps) {
  ...
  const checklistRows: ReadonlyArray<ChecklistRow> = (() => {
    ...
    const variance = computeNetOperatingIncomeVarianceRatio(noiAudit, noiDeclared);
    if (variance != null) {
      const variancePct = formatNetOperatingIncomeVariancePct(variance) ?? '0.0';
      if (variance <= 0.05) {
        noiStatus = 'ok';
        noiNoteFr =
        ...
        noiStatus = 'warn';
        noiNoteFr = `Écart de ${variancePct} % entre RNE déclaré et RNE vérifié — justifier la normalisation des dépenses.`;
        noiNoteEn = `${variancePct}% gap between declared and verified NOI — justify expense normalization.`;
      } else {
        noiStatus = 'fail';
        noiNoteFr = `Écart majeur de ${variancePct} % entre RNE déclaré et RNE vérifié — vérifier les sources avant présentation prêteur.`;
        noiNoteEn = `Major ${variancePct}% gap between declared and verified NOI — verify sources before lender submission.`;
      }
