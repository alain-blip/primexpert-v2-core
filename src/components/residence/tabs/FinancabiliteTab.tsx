...
import {
  assessNoiVerificationForFinancing,
...
export function FinancabiliteTab({ residence }: FinancabiliteTabProps) {
  ...
  const checklistRows: ReadonlyArray<ChecklistRow> = (() => {
    ...
    const noiAssessment = assessNoiVerificationForFinancing(model.noiAudit, model.noiDeclare);
    const noiStatus: ChecklistStatus = noiAssessment.status;
    const { noteFr: noiNoteFr, noteEn: noiNoteEn } = noiAssessment;
    ...
    return [
      ...
      },
      {
        ...
        status: noiStatus,
        valueFr: `${fmt(model.noiDeclare)} · ${fmt(model.noiAudit)}`,
        valueEn: `${fmt(model.noiDeclare)} · ${fmt(model.noiAudit)}`,
        noteFr: noiNoteFr,
        noteEn: noiNoteEn,
      },
    ];
