 ...
 * bancaire. OFF : NOI Déclaré (brut vendeur). ON : NOI normalisé (RBE enrichi
    ...
    >
      ...
      >
        ...
        >
          <input
            type="radio"
            id="finance-bank-noi-normalized"
            name="finance-bank-noi-basis"
            checked={optionB}
            disabled={disabled}
...
export function FinancabiliteTab({ residence }: FinancabiliteTabProps) {
  ...
  const checklistRows: ReadonlyArray<ChecklistRow> = (() => {
    ...
    const noiEvidence = assessDocumentedNoiAdequacy(noiDeclared, noiAudit);
    ...
    return [
      ...
      },
      {
        ...
        status: noiEvidence.status,
        valueFr: `${fmt(noiDeclared)} · ${fmt(noiAudit)}`,
        valueEn: `${fmt(noiDeclared)} · ${fmt(noiAudit)}`,
        noteFr: noiEvidence.noteFr,
        noteEn: noiEvidence.noteEn,
      },
    ];
  })();
