...
## PR #19 — Corrections QA règles financières et linguistiques (2026-06-01)

**Déclencheur :** PR [#19](https://github.com/alain-blip/primexpert-v2-core/pull/19) — branche `cursor/contr-le-qualit-r-gles-a0e7` (`d13dadc` → `408e951`).

**Statut :** **[EN VALIDATION HUMAINE ALAIN — documentation alignée en PR dédiée]**

| Volet | Ajouts / impacts |
|-------|------------------|
| **Nouveau module core** | `packages/core/src/financial/acmValuationPresentation.ts` exporté par `@primexpert/core/financial` ; orchestre `computeSimpleAcmValuationPresentation()` et `computeResidenceAcmValuationPresentation()` à partir du moteur `@primexpert/core/valuation`. |
| **ACM Workhub + fiche résidence** | `src/components/ACM.tsx` et `src/components/acm/AcmValuationWorkspace.tsx` consomment l'orchestrateur core pour le taux de capitalisation global (TGA), les scénarios d'occupation, le prix recommandé et la lecture vendeur. |
| **Finançabilité** | `computeFinancabilite.ts` ajoute `assessNoiVerificationForFinancing()` (`ok` / `warn` / `fail` / `unknown`) ; `FinancabiliteTab` déplace la comparaison RNE déclaré / RNE vérifié hors React. |
| **Langage Québec** | Corrections UI dans Finançabilité, Revenus & Dépenses et Vérification 360° : libellés exposés remplacent « audité/audit » par **vérifié**, **vérification** ou **moteur financier SSOT** selon le contexte. |
| **Cloud Functions** | Aucune nouvelle route ni fonction ; pas de changement `functions/src`. |
| **Schémas / Firestore** | Aucune nouvelle collection, sous-collection ou clé persistée ; consommation inchangée de `residences/{id}/financial/dataV2`, `residences/{id}` et `manualVerifications` client. |

**Règle #0 confirmée :** la présentation ACM et la vérification RNE prêteur sont centralisées dans `@primexpert/core/financial`; React ne conserve que l'état d'entrée, l'affichage et les actions HITL.

**Règle de non-duplication appliquée :** les sections existantes Finance, ACM et langage Québec sont enrichies ; aucune nouvelle section concurrente pour `financial/dataV2`, aucun alias Firestore et aucune collection parallèle ne sont introduits.
