...
- **Toute logique financière, identité, valuation, mail parser** vit dans `packages/core/` (`@primexpert/core`). Les conversions revenu net d'exploitation (RNE) ↔ valeur ↔ taux de capitalisation (TGA) passent par `packages/core/src/financial/capitalization.ts`.
...
- **Revenus & Dépenses** : prévisualisation grille (`revenusDepensesPreview.ts`) branchée sur `capitalization.ts`, suggestions normalisation (`normalizationSuggestions.ts`), benchmark marché (`marketBenchmarks.ts`, `globalFinancialBenchmark.ts`).
...
| **Anonymisation** | Variables de performance seulement : classe d'actif, prix réel, taux de capitalisation global (TGA) via `resolveNetOperatingIncome()` + `computeCapitalizationRatePct()`, prix au pi², région, ville, FSALDU-3. Purge : noms, UID, orgId, adresses exactes, cadastre, numéro d'inscription lié. |
...
## Centralisation QA — revenu net d'exploitation (RNE) et taux de capitalisation (TGA) (PR #10 — 2026-06-01)

**Statut :** **[EN REVUE QA — commits `e1bec25` → `27cf145`]**

| Élément | Détail |
|---------|--------|
| **SSOT calcul** | `packages/core/src/financial/capitalization.ts` — `resolveNetOperatingIncome()`, `computeCapitalizationRateDecimal()`, `computeCapitalizationRatePct()`, `capitalizeNoiAtCapRatePct()`. |
| **Surfaces raccordées** | Hub Finance (`revenusDepensesPreview.ts`), sauvegarde `financialDataService.ts`, ACM (`residenceAcmBootstrap.ts`), comparables Centris (`centrisComparableCapRate.ts`), flywheel interne (`internalMarketFlywheel.ts`), rendus PA Actifs / assembleur. |
| **Format canonique** | `calculatedResults.tauxCapitalisation` demeure un ratio décimal (ex. `0.207`) ; les sorties UI / marché nommées `tgaPct` ou `capRatePct` demeurent en pourcentage (ex. `20.70`). |
| **Prebuild Functions** | `sync-core-analytics-flywheel.cjs` vendore maintenant `financial/capitalization.ts` vers `functions/src/analytics/_vendored/capitalization.ts` au lieu de réimplanter une formule locale. |
| **Règle anti-dérive** | Plus de calcul direct `RNE / prix` dans React ou dans une Function : appeler le module core et conserver la distinction décimal vs pourcentage. |

**HITL :** la centralisation sécurise la cohérence des montants, mais le courtier titulaire de permis valide toujours les hypothèses financières avant toute diffusion, promesse ou recommandation de valeur.
...
*Journal mis à jour : 2026-06-01 — PR #10 : centralisation RNE / taux de capitalisation (TGA), flywheel et formats Firestore clarifiés.*
