...
## PR #31 — Centralisation revenu net d'exploitation (RNE) / taux de capitalisation (TGA) UI et garde QA transaction RPA (2026-06-01)

**Statut :** **[DRAFT — validation humaine Alain requise — HEAD `259504e`]**  
**Titre PR :** `Draft: centraliser les calculs RNE/TGA UI`

| Élément | Détail |
|---------|--------|
| **Règle #0** | Nouveau module SSOT `packages/core/src/financial/capitalization.ts` — toutes les conversions revenu net d'exploitation (RNE) ↔ valeur ↔ taux de capitalisation (TGA) passent par le core. |
| **Helpers core** | `resolveNetOperatingIncome`, `computeCapitalizationRateDecimal`, `computeCapitalizationRatePct`, `normalizeCapRateDecimal`, `normalizeCapRatePct`, `addCapRatePctAdjustments`, `capitalizeNoiAtCapRatePct`, `computeNoiVariancePct`. |
| **Surfaces alignées** | Hub Finance (`revenusDepensesPreview`, `computeFinancabilite`, `financialOptimization360`), ACM (`residenceAcmBootstrap`, `marketCapRate`, `AcmValuationWorkspace`), rapports PDF/HTML (`acmPresentationReport`, `acmVendorPdfService`, `buildPaActifsRenderData`), moteur marché (`centrisComparableCapRate`, `internalMarketFlywheel`). |
| **Functions** | Vendoring `functions/src/analytics/_vendored/capitalization.ts` via `functions/scripts/sync-core-analytics-flywheel.cjs` ; les calculs flywheel n'ont plus de formule RNE/TGA locale. |
| **Firestore** | Aucune collection nouvelle ; enrichissement des documents existants `residences/{id}/financial/dataV2.calculatedResults` (`revenuNetExploitation`, `tauxCapitalisation`, `depensesTotalesNormalisees`) et `residences/{id}.promesseAchat` (délais PA). |
| **QA automatisée** | Ajout de Vitest + workflow `.github/workflows/rpa-transaction-test-coverage.yml` ; commande `npm run test:rpa-coverage` ; script `scripts/check-resolveColumnId-coverage.mjs`. |
| **Kanban RPA** | `resolveColumnId()` couvert à 100 % : slugs Firestore, statuts bruts actifs, mapping legacy Copilote/FR, variantes RESO/Centris mappables. |
| **Promesse d'achat acceptée** | `PA_ACCEPTEE_CRITICAL_DEADLINE_KEYS` verrouille 7 échéances : réponse, visite lieux, vérification documents, inspection, financement, permis, dédit Loi sur le courtage immobilier art. 73.2 (3 jours). |

**Décision :** les composants React et services front ne calculent plus directement le taux de capitalisation (TGA) ni les écarts de revenu net d'exploitation (RNE). Ils appellent les helpers `@primexpert/core/financial`, ce qui évite les divergences entre UI, PDF, assembleur de contrat, ACM et Functions.

**HITL :** la PR demeure en brouillon ; aucune publication ni merge sans validation d'Alain. Le courtier conserve la responsabilité de vérifier les hypothèses financières et les échéances légales avant diffusion ou signature.
