...
- **PR #47 (Finançabilité)** : les libellés client de `FinancabiliteTab.tsx` emploient **RNE normalisé** / sources vérifiées (`normalized NOI`, `verified expenses`) ; aucune occurrence « audité/audited/audit sources » ne doit réapparaître dans cette surface.
...
## PR #47 — Purge lexicale Finançabilité Hub Finance (2026-06-01)

**Déclencheur :** PR [#47](https://github.com/alain-blip/primexpert-v2-core/pull/47) — branche `cursor/contr-le-qualit-r-gles-5057`, base `main`.

**Statut :** **[EN VALIDATION HUMAINE ALAIN — documentation alignée en PR dédiée brouillon]**

| Axe analysé | Résultat |
|-------------|----------|
| **Nouveaux modules / packages** | Aucun ; aucun export `@primexpert/core` ajouté par ce diff. |
| **UI modifiée** | `src/components/residence/tabs/FinancabiliteTab.tsx` seulement : remplacement des libellés « RNE audité/audited NOI » par **RNE normalisé / normalized NOI** et des consignes par **vérifier / validate / verified expenses**. |
| **Routes Cloud Functions** | Aucune nouvelle route, callable, trigger ou région. |
| **Firestore / règles / modèles** | Aucun nouveau champ, aucune sous-collection, aucun changement `firestore.rules` ; `docs/project_canonical_fields.md` reste inchangé pour PR #47 hors enrichissements déjà documentés PR #9. |

**Règle de non-duplication appliquée :** la décision est fusionnée dans la règle linguistique existante et dans les entrées Hub Finance ; aucune section « lexique finance » parallèle ni nouveau dictionnaire n'est créé.

**HITL :** validation humaine Alain obligatoire avant publication ; la PR de documentation demeure brouillon et ne contient que les fichiers docs autorisés.
...
*Journal mis à jour : 2026-06-01 — PR #9 documentée sans dupliquer l'historique PR #3 ; PR #47 consigne la purge lexicale Finançabilité sans changement Firestore.*
