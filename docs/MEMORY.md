...
## PR #28 — Correction QA calcul ACM via core financier (2026-06-01)

**Déclencheur :** PR [#28](https://github.com/alain-blip/primexpert-v2-core/pull/28) — branche `cursor/contr-le-qualit-r-gles-d1a9` (`d13dadc` → `cd01d0f`).

**Statut :** **[EN VALIDATION HUMAINE ALAIN — documentation alignée en PR dédiée]**

| Volet | Ajouts / impacts |
|-------|------------------|
| **Nouveau module core** | `packages/core/src/financial/acmValuationPresentation.ts` centralise la présentation ACM : calcul simple, calcul résidence, ajustement du taux de capitalisation global (TGA), stress 85/90/100 et prix recommandé. |
| **Barrel financier** | `packages/core/src/financial/index.ts` exporte `acmValuationPresentation`, donc les UI consomment `@primexpert/core/financial` au lieu de réassembler la logique dans React. |
| **ACM Workhub** | `src/components/ACM.tsx` appelle `computeSimpleAcmValuationPresentation()` ; le taux effectif alimente aussi la Lecture Vendeur déterministe. |
| **ACM fiche résidence** | `AcmValuationWorkspace.tsx` appelle `computeResidenceAcmValuationPresentation()` et `computeAcmManualPricingSuggestionValues()` ; les suggestions multi-angles restent HITL dans `manualVerifications`. |
| **Finançabilité** | `computeFinancabilite.ts` expose `assessNoiVerificationForFinancing()` ; `FinancabiliteTab` supprime la checklist RNE dupliquée et parle de RNE vérifié plutôt que RNE audité côté UI. |
| **Libellés finance** | `Analyse360FinanceTab` et `RevenusDepensesTab` reformulent les messages autour du moteur financier SSOT sans exposer de formule locale parallèle. |
| **Firestore** | Aucun nouveau champ ni sous-collection : la correction consomme `residences/{id}`, `residences/{id}/financial/dataV2`, `market_analytics_raw`, `marketSnapshots/v1` et `manualVerifications` existants. |

**Règle #0 :** les calculs ACM de présentation (TGA effectif, scénarios d'occupation, prix recommandé, suggestions marché/performance/potentiel) sont maintenant portés par le core financier ; React orchestre l'affichage et la validation humaine seulement.

**Règle de non-duplication appliquée :** les blocs ACM existants sont enrichis ; aucun nouveau silo Firestore `acm`, aucun duplicat de formule `RNE ÷ TGA` dans les composants et aucune section documentaire parallèle n'est créée.

**HITL :** les prix suggérés demeurent des aides à la décision. Le courtier titulaire de permis valide le taux retenu, les sources RNE/RBE et la présentation vendeur avant diffusion.
...
*Journal mis à jour : 2026-06-01 — PR #3 et PR #28 documentées (couverture RPA, Centris/off-market, flywheel/OER, WORM canonique, ACM via core financier).*
