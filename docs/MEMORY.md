...
**Statut :** **[CODÉ PR #10 — validation humaine Alain requise]** *(numéro de version distinct du jalon assembleur V3.5 scellé ci-dessous)*
...
**Statut :** **[CODÉ PR #10 — validation humaine Alain requise]**
...
## Session 2026-06-01 — Alignement Bible sur PR #10 « correctifs QA RNE TGA centralisés »

**Statut :** **[DOCUMENTATION BROUILLON — validation humaine Alain requise avant merge]**
**Déclencheur analysé :** PR #10 `cursor/contr-le-qualit-r-gles-9dc3` → `main`, diff `d13dadc...e1bec25`.

| Axe | Éléments documentés |
|-----|---------------------|
| **QA RNE/TGA centralisés** | Commit `e1bec25` : `@primexpert/core/financial/capitalization.ts`, helpers `computeCapitalizationRatePct`, `computeCapitalizationRateDecimal`, `capitalizeNoiAtCapRatePct`; tests RPA et PA acceptée via Vitest. |
| **Modules core nouveaux/étendus** | `@primexpert/core/analytics`, `security`, `forms`; extensions `financial`, `market`, `residence`, `transaction`. |
| **Cloud Functions** | `onVaultDocumentWrite`, `onTransactionConcludedFlywheel`, `centrisListingsSyncNightly`; `marketDocumentParseIA` ramené à **512 MiB / 60 s** avec slice sémantique + cache MD5. |
| **UI fiche résidence** | `ResidenceDataContext`, `ResidenceTabErrorBoundary`, `LegalVaultWormPanel`, `ContractAssemblerPanel`, `TerritorialCentrisCompetitionSection`, création inscription hors marché et statut Centris verrouillé. |
| **Firestore / règles** | `organizations/{orgId}/legal_vault`, sous-collection `compliance_logs`, `listings_cache`, `market_documents.orgId`, `market_macro_stats.orgId`, `market_analytics_raw.dataSource = internal_flywheel`, `residences.listingSource` / `isManuallyOverridden`. |
| **Périmètre de soumission** | Documentation seulement : `docs/README.md`, `docs/MEMORY.md`, `docs/arborescence.md`, `docs/project_canonical_fields.md`. Aucun push direct sur `main`, aucun merge, attente validation Alain. |

**Décision de non-duplication :** les champs Firestore détaillés restent dans `project_canonical_fields.md`; l’arborescence ne conserve que les chemins et fonctions; le README ne garde que les statuts exécutifs; ce journal conserve le contexte de PR.

---

*Journal mis à jour : 2026-06-01 — Alignement PR #10 : RNE/TGA centralisés, flywheel/Centris, WORM et champs canoniques Firestore.*
