...
- **Correctif PR #29 (2026-06-01)** : les libellés visibles finance emploient **RNE vérifié** / *verified NOI* ; les formulations « RNE audité », « dépenses auditées » et les formules affichées comme décision client sont remplacées par une référence au moteur financier central.
...
- **QA PR #29 (2026-06-01)** : helpers TGA SSOT dans `revenusDepensesPreview.ts` — `normalizeTgaPct`, `computeTgaRatioFromRneAndPrice`, `computeTgaPctFromRneAndPrice`, `computeCapitalizedValueFromRneAndTga` ; `financialDataService.ts` écrit le taux de capitalisation comme ratio canonique, l’UI convertit en pourcentage au rendu.
...
## PR #29 — Corrections QA règles RPA finance et lexique (2026-06-01)

**Déclencheur :** PR [#29](https://github.com/alain-blip/primexpert-v2-core/pull/29) — branche `cursor/contr-le-qualit-r-gles-7615` (`d13dadc` → `dcad536`).

**Statut :** **[EN VALIDATION HUMAINE ALAIN — documentation alignée en PR dédiée]**

| Volet | Ajouts / impacts |
|-------|------------------|
| **Nouveau module/package** | Aucun nouveau package. Extension du module existant `@primexpert/core/financial` via `revenusDepensesPreview.ts`. |
| **Core finance** | Ajout des helpers `normalizeTgaPct`, `computeTgaRatioFromRneAndPrice`, `computeTgaPctFromRneAndPrice`, `computeCapitalizedValueFromRneAndTga` pour éviter les formules TGA dispersées. |
| **UI ACM** | `ACM.tsx` et `AcmValuationWorkspace.tsx` affichent « Prix recommandé par le moteur financier SSOT » ; le calcul de valeur capitalisée passe par le helper core. |
| **UI Finançabilité / Analyse 360°** | Purge des libellés visibles « RNE audité » / *audited NOI* au profit de **RNE vérifié** / *verified NOI* ; le message d’impact de valeur renvoie au moteur financier central. |
| **Service Firestore finance** | `financialDataService.ts` réutilise `computeTgaRatioFromRneAndPrice()` pour écrire `calculatedResults.tauxCapitalisation` sans logique parallèle. |
| **Routes Cloud Functions** | Aucune route ajoutée ou modifiée. |
| **Schéma Firestore** | Aucune nouvelle clé ni sous-collection ; précision documentaire : `calculatedResults.tauxCapitalisation` reste un ratio stocké, normalisé en pourcentage par le core à l’affichage. |

**Règle de non-duplication appliquée :** les corrections prolongent les sections Finance, ACM et lexique existantes ; aucun dictionnaire TGA, collection finance parallèle ni nouveau moteur UI n'est créé.

**HITL :** la PR de documentation demeure en brouillon jusqu'à validation d'Alain ; aucun merge ni push direct sur `main`.
