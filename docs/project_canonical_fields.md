...
**Capitalisation financière (PR #31)** : `packages/core/src/financial/capitalization.ts` — SSOT revenu net d'exploitation (RNE) / taux de capitalisation (TGA), vendoré dans `functions/src/analytics/_vendored/capitalization.ts`.
...
### Clés `calculatedResults` — capitalisation centralisée PR #31

Ces champs restent dans le document existant `financial/dataV2` ; aucune sous-collection nouvelle n'est créée.

| Clé | Type | Source canonique |
|-----|------|------------------|
| `revenuNetExploitation` | number \| null | `resolveNetOperatingIncome()` — direct si valide, sinon `revenuBrutEffectif - depensesExploitation` |
| `depensesTotalesNormalisees` | number | `sumNormalizedOperatingExpenses()` puis RNE centralisé |
| `tauxCapitalisation` | number \| null | `computeCapitalizationRateDecimal(revenuNetExploitation, prixDemande)` — décimal Firestore (ex. `0.207`) |
| `valeurCapitalisation` / `valeurBanquable` | number \| null | `capitalizeNoiAtCapRatePct()` quand un TGA de référence est disponible |

Surfaces d'écriture : `saveExpenseAdjustmentsToFinancial()` et `saveManualFinancialEntry()` enrichissent `calculatedResults` via les helpers core ; les composants React ne recalculent pas ces formules localement.
...
| `promesseAchat.dateReception` | string | Date de réception de la PA (ISO `yyyy-mm-dd`) |
| `promesseAchat.delaiReponseJours` | number \| null | Délai de réponse en jours ; alimente `dateLimiteReponse` |
...
| `promesseAchat.dateLimiteDeduitLci` | string | Calculée depuis `dateAcceptation + 3 jours` — Loi sur le courtage immobilier art. 73.2 |
...
### PA acceptée — 7 échéances critiques (PR #31)

SSOT : `packages/core/src/transaction/promesseAchatEngine.ts`.

| Constante / validateur | Rôle |
|------------------------|------|
| `PA_ACCEPTEE_CRITICAL_DEADLINE_KEYS` | Liste stricte des 7 clés : réponse, visite lieux, vérification documents, inspection, financement, permis, dédit LCI |
| `DEDIT_LCI_ART_73_2_JOURS` | Délai légal fixe de 3 jours |
| `validatePaAccepteeCriticalDeadlines()` | Échec si une PA `accepted` ne peut pas produire les 7 échéances |

Tests : `packages/core/src/transaction/__tests__/paAccepteeCriticalDeadlines.test.ts` ; exécution par `npm run test:rpa-coverage` et workflow `RPA Transaction Test Coverage`.
...
*Dernière mise à jour : 2026-06-01 — PR #31 Draft : capitalisation RNE/TGA centralisée + 7 échéances PA acceptée protégées (`259504e`).*
