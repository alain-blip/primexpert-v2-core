...
**PR #19 — aucune mutation de schéma :** les helpers `computeSimpleAcmValuationPresentation()`, `computeResidenceAcmValuationPresentation()` et `assessNoiVerificationForFinancing()` lisent les blocs existants (`calculatedResults`, `baseData`, hints résidence) et retournent des view models TypeScript. Ils ne créent ni champ `acm`, ni sous-collection, ni persistance automatique du prix recommandé.
...
| View models finance | `packages/core/src/financial/*.ts` dont `acmValuationPresentation.ts` |
...
*Dernière mise à jour : 2026-06-01 — PR #19 : QA finance/langage, helpers ACM/RNE sans nouvelle structure Firestore.*
