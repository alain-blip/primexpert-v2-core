...
### QA PR #14 — champs financiers recalculés sans collection parallèle

| Chemin | Type | Description |
|--------|------|-------------|
| `baseData.expenseAdjustments` | map | Ajustements CPA / normalisés par clé `EXPENSE_KEYS` + `autresDepenses[]` + `verified`; sauvegardés par `saveExpenseAdjustmentsToFinancial()`. |
| `baseData.depenses` | map `DepensesGrid` | Grille dépenses fusionnée (saisie manuelle + import IA), jamais remplacée par une collection parallèle. |
| `baseData.financement` | map | Saisie institutionnelle (`soldeHypothecaire`, `tauxInteret`, `paiementMensuel`, `amortissement`, `dscr`, `tgaPreteur`). |
| `calculatedResults.depensesTotalesNormalisees` | number | Total des dépenses d'exploitation après ajustements; source des scénarios RNE vérifié. |
| `calculatedResults.revenuNetExploitation` | number \| null | Recalculé via `recomputeFinancialCalculatedResults()` après ajustement ou saisie manuelle. |
| `calculatedResults.tauxCapitalisation` | number | TGA décimal normalisé par `computeCapitalizationRateFromNoi(RNE, prixDemande)`; non écrit si RNE ou prix invalide. |
| `calculatedResults._source` | string | `manual_entry` ou `ai_extraction` selon provenance de la saisie validée humainement. |
| `calculatedResults._confidence` | string | `validation_required` ou `human_validated`; ne remplace pas la revue courtier. |
| `lastInjection` | map | `{ source: 'human_validated_ia', documentId, atMillis }` après préremplissage IA validé. |

> **Règle TGA :** toute valeur entrée comme pourcentage (`8.5`) ou décimal (`0.085`) passe par `normalizeCapitalizationRate()` avant usage ACM, comparables, rapport vendeur ou écriture Firestore.
...
| View models finance | `packages/core/src/financial/*.ts` dont `capitalizationMetrics.ts` (QA TGA/RNE PR #14) |
...
| QA capitalisation / TGA | `capitalizationMetrics.ts`, `valuationEngine.ts`, `marketCapRate.ts`, `lenderCapRate.ts`, `stressTest.ts`, `financialDataService.ts`, `extractedDataInjection.ts` |
...
*Dernière mise à jour : 2026-06-01 — PR #14 : `capitalizationMetrics`, recalcul `financial/dataV2`, TGA normalisé, dépenses normalisées et lexique RNE vérifié.*
