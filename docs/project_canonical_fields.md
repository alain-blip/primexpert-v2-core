...
|--------------|-----------------|
| `calculatedResults.tauxCapitalisation` | Peut être stocké comme **ratio décimal** (`0.207`) lors des ajustements dépenses ; ne pas l'afficher directement. |
| Affichage TGA | Toujours passer par `normalizeTgaPct()` pour accepter ratio (`0.207`) ou pourcentage (`20.7`) et rendre un pourcentage utilisateur. |
| Recalcul depuis RNE/prix | `computeTgaRatioFromRneAndPrice(rne, prixDemande)` retourne le ratio persistant ; `computeTgaPctFromRneAndPrice()` retourne la valeur UI en %. |
| Valeur capitalisée | `computeCapitalizedValueFromRneAndTga(rne, tgaPct)` centralise `RNE / (TGA / 100)` ; React ne recode pas la formule. |

**Lexique UI finance :** afficher **RNE vérifié** / *verified NOI* (pas « RNE audité ») et référer au **moteur financier central** lorsque le prix recommandé ou l'impact de valeur est présenté au client.
...
| View models finance | `packages/core/src/financial/*.ts`, `revenusDepensesPreview.ts` (helpers TGA ratio/%/capitalisation) |
