...
**Finance — RNE / taux de capitalisation (TGA)** : `packages/core/src/financial/capitalization.ts` centralise `resolveNetOperatingIncome`, `computeCapitalizationRateDecimal`, `computeCapitalizationRatePct`, `capitalizeNoiAtCapRatePct` ; `calculatedResults.tauxCapitalisation` est un ratio décimal, les champs `tgaPct` / `capRatePct` sont des pourcentages.  
...
## Sous-collection `organizations/{orgId}/legal_vault/{documentId}`

Coffre-fort légal WORM V3.0 — règles Firestore : création par membre de l'organisation, verrouillage final irréversible (`false → true`), suppression interdite.

| Champ | Type | Description |
|--------|------|-------------|
| `orgId` | string | Organisation propriétaire |
| `brokerId` | string | Courtier responsable (`request.auth.uid` à la création) |
| `documentId` | string | ID déterministe du document vault |
| `documentType` | string | `CONTRAT_COURTAGE` \| `PROMESSE_ACHAT` \| `FICHE_DESCRIPTIVE` \| `ACM_REPORT` |
| `storageUrl` | string | URL ou chemin de conservation |
| `createdAtMillis` | number | Horodatage création |
| `oaciqRetentionExpiryTimestamp` | number | Expiration rétention OACIQ (6 ans / 2190 j selon type) |
| `isFinalWormLocked` | bool | `false` au brouillon ; seule mutation client autorisée vers `true` |
| `lockedAtMillis` | number | Horodatage verrouillage final |
| `lastWriteClientIp` | string | IP résolue côté client/service au moment du verrouillage |

### Sous-collection `organizations/{orgId}/legal_vault/{documentId}/compliance_logs/{entryId}`

Journal d'adéquation de conformité légale — lecture membres org, écriture serveur uniquement (`onVaultDocumentWrite` / Admin SDK).

| Champ | Type | Description |
|--------|------|-------------|
| `eventType` | string | `READ` \| `WRITE` \| `LOCK` \| `EXPORT_ZIP` selon le writer |
| `actorUid` | string | Utilisateur ou service ayant déclenché l'événement |
| `atMillis` | number | Horodatage append |
| `documentId` | string | Document vault parent |
| `previousHash`, `entryHash` | string | Chaînage SHA-256 du journal |
...
| `statut` | string | Statut courtage affichable / legacy (`actif`, `pa-acceptee`, `vendue`, `expiree`, `annulee`, `suspendue`) synchronisé par `buildInscriptionBrokerageStatusPatch()` |
| **`listingSource`** | string | `centris` (défaut historique) \| `off_market` — les fiches hors marché sont exclues de la synchronisation Centris et portent les filigranes confidentiels |
| **`isManuallyOverridden`** | bool | Si `true`, la synchronisation descendante MLS/Centris ne remplace pas le statut courtier |
...
| **`internalFlywheelIngestion`** | map | Marqueur idempotence Data Flywheel — `promiseAtMillis`, `soldAtMillis` (Admin SDK / Function seulement) |
...
| RNE / valeur / TGA | `capitalization.ts` — `resolveNetOperatingIncome()`, `computeCapitalizationRateDecimal()`, `computeCapitalizationRatePct()`, `capitalizeNoiAtCapRatePct()` |
...
**Format obligatoire TGA :** `calculatedResults.tauxCapitalisation` reste un ratio décimal (`0.207`) pour les calculs financiers ; `tgaPct` / `capRatePct` restent des pourcentages (`20.70`) dans les vues marché, ACM et `market_analytics_raw`.
...
| **`organizations/{orgId}/legal_vault`** | `orgId` | Coffre-fort WORM OACIQ — verrouillage final irréversible |
| **`organizations/{orgId}/legal_vault/{documentId}/compliance_logs`** | `orgId` | Journal SHA-256 append-only (serveur uniquement) |
| **`vendor_portal_invites/{token}`** | `brokerId` | Jetons portail vendeur autonome (30 j) |
| **`listings_cache/{entryId}`** | — | Cache Centris Matrix — lecture authentifiée, écriture serveur |
...
Cache Centris / Matrix utilisé par la concurrence territoriale ACM ; lecture client authentifiée, écriture serveur uniquement.

| Champ | Type | Description |
|--------|------|-------------|
| `source` | string | `centris_odata` pour entrées issues du connecteur Centris |
| `standardStatus` | string | Statut RESO / MLS (`Active`, `Pending`, `Closed`, etc.) |
| `canonicalPreview` | map | Projection UI / ACM (localisation, prix, unités, finances) |
| `financials` | map | RBE, dépenses, revenu net d'exploitation (RNE), taux de capitalisation (TGA) si disponible |
| `updatedAtMillis` | number | Horodatage de synchronisation |
...
| **`dataSource`** | string | `internal_flywheel` pour alimentation anonyme interne ; absent ou autre selon injections historiques |
| `siloType` | string | ex. `rpa_ri_chsld` |
| `regionAdministrative` | string | Région |
| `regionDisplayName` | string | Ville / libellé régional affichable (flywheel) |
| `postalFsa3` | string | FSALDU-3 anonymisé (trois premiers caractères du code postal, sans adresse civique) |
...
| `marketTransactionMeta` | map | Métadonnées transaction (`dateTransaction`, `typeImmeuble`, `transactionKind`, `nbPortes`, `prixParPi2`, `marketDocumentId`) |
...
> **Anti-doublons :** réinjection du même PDF, rapports chevauchants ou flywheel interne → `set(..., { merge: true })` sur l'ID empreinte ; pas de `add()` aveugle. Legacy : `packages/core/src/market/marketDeduplication.ts` (adresse normalisée + prix + date ±3 jours pour détection UI). Le `capRatePct` est toujours en pourcentage et doit provenir de `computeCapitalizationRatePct()`.
...
| Capitalisation RNE / TGA | `packages/core/src/financial/capitalization.ts`, `src/services/financialDataService.ts`, `centrisComparableCapRate.ts`, `internalMarketFlywheel.ts` |
...
| Source inscription / hors marché | `listingSource.ts`, `inscriptionBrokerageStatus.ts`, `CreateInscriptionForm.tsx`, `InscriptionStatusDropdown.tsx` |
...
**Defaults :** `buildContractAssemblerDefaults()` — prix annexe depuis revenu net d'exploitation (RNE) ÷ taux de capitalisation global (TGA) ACM (`resolveCanonicalRne`, `bootstrapResidenceAcm`, `capitalizeNoiAtCapRatePct`).
...
*Dernière mise à jour : 2026-06-01 — PR #10 : centralisation RNE / taux de capitalisation (TGA), flywheel et champs Firestore marché.*
