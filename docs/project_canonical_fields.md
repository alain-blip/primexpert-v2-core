...
**Présentation ACM (PR #28)** : `packages/core/src/financial/acmValuationPresentation.ts` — calculs de présentation sans nouvelle persistance Firestore.
...
| Présentation ACM | `computeResidenceAcmValuationPresentation()` — consomme le bootstrap ACM et retourne TGA effectif, stress 85/90/100, prix recommandé ; **n'écrit aucun champ `acm`** |
| Checklist RNE prêteur | `assessNoiVerificationForFinancing()` — compare RNE vérifié et RNE déclaré pour l'onglet Finançabilité ; statut UI non persisté |
...
| Présentation ACM core financier | `acmValuationPresentation.ts`, `AcmValuationWorkspace.tsx`, `ACM.tsx` |
...
### Calculs de présentation ACM — core financier PR #28

| Fonction core | Entrées persistées lues | Sortie UI |
|---------------|-------------------------|-----------|
| `computeResidenceAcmValuationPresentation()` | `ResidenceAcmBootstrap` construit depuis `residences/{id}` + `financial/dataV2` + GPS marché | `result`, `effectiveCapRate`, `tgaAdjustment`, `stressSummary`, `recommendedPrice` |
| `computeSimpleAcmValuationPresentation()` | Formulaire ACM Workhub isolé (non lié à une fiche) | Même structure de résultat, sans écriture Firestore |
| `computeAcmManualPricingSuggestionValues()` | RNE, unités, `TerritorialAcmMedians`, scénario 100 % | Suggestions `marketAligned`, `performanceBased`, `maxPotential` injectées dans `manualVerifications.pricingSuggestions` HITL |

> **Alignement Firestore :** la PR #28 n'ajoute pas de collection, sous-collection ni clé persistée. Les valeurs ci-dessus sont des résultats calculés en mémoire à partir des champs canoniques existants.
...
*Dernière mise à jour : 2026-06-01 — PR #3 + PR #28 : WORM, Centris/off-market, flywheel/OER, `contentHashMd5`, 7 délais PA acceptée, calcul ACM de présentation via core financier sans nouvelle clé Firestore.*
