...
| Inscriptions Centris / hors marché + concurrence territoriale | **BROUILLON PR #4 — validation Alain requise** — `listingSource`, sync Centris nocturne, override manuel, comparables ACM |
| Data Flywheel & ratio dépenses/revenus (RDE/OER) | **BROUILLON PR #4 — validation Alain requise** — ingestion anonymisée `market_analytics_raw`, médianes RDE, QA RPA |
