...
**Cycle technique :** **PR #10 — QA RNE/TGA centralisés (`e1bec25`)** sur socle V3.5–V3.7 (assembleur natif, WORM, OER, Centris, flywheel) · validation humaine Alain requise avant publication
...
| Data Flywheel, Centris & inscriptions hors marché | **CODÉ PR #10 — validation humaine requise** (`onTransactionConcludedFlywheel`, `centrisListingsSyncNightly`) |
| Tests QA RPA — délais PA acceptée & colonnes pipeline | **CÂBLÉ CI — Vitest** (`resolveColumnId`, délais critiques transaction) |
...
3. **Finance** — Document unique `residences/{id}/financial/dataV2` ; contexte `FinancialDataProvider` ; prix/RNE/TGA via `@primexpert/core/financial` (`resolveCanonicalRne`, `capitalization.ts`) + `ResidenceDataContext`.
...
13. **Statistiques du marché** — Vault `market_documents` multi-tenant (`orgId`) ; découpage sémantique PDF + cache `contentHashMd5` ; injection idempotente vers `market_analytics_raw` / `market_macro_stats` (`marketDeduplication.ts`).
14. **Analyse de mise en marché (ACM)** — Fiche résidence, onglet Marché : bootstrap `residenceAcmBootstrap.ts`, workspace `AcmValuationWorkspace`, TGA médian GPS/Centris par région/classe, recalcul live.
...
24. **RNE/TGA centralisés** — Toute surface financière ou ACM consomme les helpers core (`computeCapitalizationRatePct`, `capitalizeNoiAtCapRatePct`) ; aucun recalcul divergent dans React ou Functions.
...
| **Mes inscriptions** | `Listings.tsx` | Pipeline Kanban 4 colonnes, DnD, filtres régions, recherche, création hors marché + statut Centris verrouillé | LIVE ; PR #10 en validation |
...
| **Statistiques du marché** | `MarketLibraryDashboard` | `market_documents`, parse Vertex sémantique, cache MD5, injection HITL, flywheel anonymisé | LIVE ; PR #10 en validation |
...
| Parse | `marketDocumentParseIA` — **512 MiB**, **60 s** ; découpage sémantique `marketPdfSlice.ts` + cache `contentHashMd5` |
| Injection | `injectMarketMacroStats` — `orgId` propagé, empreintes déterministes, `set(merge: true)` |
| Flywheel / Centris | `onTransactionConcludedFlywheel`, `centrisListingsSyncNightly`, `marketAnalyticsService.ts`, `TerritorialCentrisCompetitionSection` |
| Anti-doublons | `packages/core/src/market/marketDeduplication.ts` + `internalFlywheelFingerprint` |
...
| Marché | ✅ **Analyse de mise en marché (ACM)** (SSOT finances + TGA GPS/Centris centralisé) + concurrence territoriale |
...
| Promesse | ✅ Cockpit PA (`offre` SSOT), délais critiques PA acceptée testés + **assembleur contrat V3.5** (`ContractAssemblerPanel`, export HTML) |
...
*Index mis à jour : 2026-06-01 — Alignement PR #10 : QA RNE/TGA centralisés, flywheel/Centris, WORM et docs Firestore.*
