...
**Cycle technique :** **V3.8 — QA RNE / TGA centralisés** + **V3.7 analytics marché (RDE/OER)** + **V3.5 assembleur de mandats natif scellé** (juin 2026) · architectures sécurisées V3.0 en production
...
| QA RNE / TGA centralisés | **DOCUMENTÉ — PR #10** (`capitalization.ts`, `resolveCanonicalRne.ts`, `financialDataService.ts`) |
...
13. **Statistiques du marché** — Vault `market_documents` ; cache MD5 + découpage sémantique PDF ; injection idempotente vers `market_analytics_raw` / `market_macro_stats` (`marketDeduplication.ts`) ; ratio dépenses / revenus (RDE/OER).
14. **Analyse de mise en marché (ACM)** — Fiche résidence, onglet Marché : bootstrap `residenceAcmBootstrap.ts`, workspace `AcmValuationWorkspace`, TGA médian GPS par région/classe, concurrence `listings_cache` + `market_analytics_raw`, recalcul live.
...
24. **Data Flywheel & Centris** — `onTransactionConcludedFlywheel` anonymise les transactions conclues vers `market_analytics_raw` ; `centrisListingsSyncNightly` synchronise les statuts MLS sans toucher aux inscriptions `off_market`.
...
| **Statistiques du marché** | `MarketLibraryDashboard` | `market_documents`, cache MD5, RDE/OER, parse Vertex, injection HITL | LIVE |
...
| Cache & découpage | `contentHashMd5`, `marketPdfSlice.ts`, `marketPdfSemanticAnchors.ts` — clone d'extraction si PDF identique |
...
| Analytics | `@primexpert/core/analytics/marketMetrics.ts` — ratio des dépenses d'exploitation (RDE/OER) |
| Flywheel | `onTransactionConcludedFlywheel` — données anonymisées internes (`dataSource: internal_flywheel`) |
...
| Marché | ✅ **Analyse de mise en marché (ACM)** (SSOT finances + TGA GPS centralisé) + concurrence territoriale Centris / flywheel |
| Documents | ✅ Scan + parse Vertex + distribution + Verrouillage WORM/OACIQ client (V3.1) + journal de conformité Montréal |
...
*Index mis à jour : 2026-06-01 — QA RNE / TGA centralisés, analytics marché RDE/OER et alignement Firestore PR #10.*
