...
**Source unique avec le code :** `01_PRIMEXPERT_SYSTEME_APP_STABLE_V2/docs/`<br>
**URL officielle :** https://primexpert-app-v2.web.app<br>
...
| Hub Finance — cohérence RNE / prix inter-onglets | **DÉPLOYÉ PROD — commit `d232673`** + **QA PR #14** (capitalisation SSOT, dépenses normalisées) |
...
3. **Finance** — Document unique `residences/{id}/financial/dataV2` ; contexte `FinancialDataProvider` ; prix/RNE/TGA via `@primexpert/core/financial` (`capitalizationMetrics`) + `ResidenceDataContext`.
...
14. **Analyse de mise en marché (ACM)** — Fiche résidence, onglet Marché : bootstrap `residenceAcmBootstrap.ts`, workspace `AcmValuationWorkspace`, TGA médian GPS par région/classe, recalcul live et valeur capitalisée via helper financier partagé.
...
| **QA finance / lexique** | `FinancabiliteTab` + `AcmValuationWorkspace` | `capitalizationMetrics`, recalcul `financial/dataV2`, libellés RNE vérifié | EN REVUE PR #14 |
...
| Finances (Hub 5 sous-onglets) | ✅ + benchmark global + QA RNE/TGA normalisée |
...
*Index mis à jour : 2026-06-01 — alignement PR #14 (QA finance, TGA partagé, dépenses normalisées, lexique vérifié) dans la continuité PR #3.*
