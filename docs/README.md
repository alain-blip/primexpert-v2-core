...
**Cycle technique :** **V3.7 — marché/OER/flywheel + assembleur de mandats V3.5** · **couverture RPA PR #3** (Kanban + délais PA acceptée) · **QA calcul ACM PR #28** (présentation via core financier) · architectures sécurisées V3.0 en production
...
| Analyse de mise en marché (ACM) & benchmark taux de capitalisation (TGA) | **OPÉRATIONNEL — PRODUCTION LIVE** · QA présentation core **EN REVUE PR #28** |
...
14. **Analyse de mise en marché (ACM)** — Fiche résidence, onglet Marché : bootstrap `residenceAcmBootstrap.ts`, présentation `acmValuationPresentation.ts` dans `@primexpert/core/financial`, workspace `AcmValuationWorkspace`, TGA médian GPS par région/classe, scénarios d'occupation et prix recommandé sans calcul métier dans React.
...
| Finances (Hub 5 sous-onglets) | ✅ + benchmark global + adéquation RNE documenté côté prêteur |
| Déclaration | ✅ Questionnaire OACIQ |
| Marché | ✅ **Analyse de mise en marché (ACM)** (SSOT finances + TGA GPS + présentation core financier PR #28) + concurrence territoriale |
...
*Index mis à jour : 2026-06-01 — alignements PR #3 et PR #28 (couverture RPA, Centris/off-market, flywheel/OER, coffre-fort WORM, calcul ACM via core financier).*
