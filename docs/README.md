...
**Cycle technique :** **V3.5 — assembleur de mandats natif scellé** + **redressement finance SSOT fiche résidence (`d232673`)** + **verrou QA RNE/TGA (`259504e`, PR #31 Draft)** (mai-juin 2026) · architectures sécurisées V3.0 en production
...
| Calculs revenu net d'exploitation (RNE) / taux de capitalisation (TGA) & garde QA PA | **EN REVUE DRAFT — PR #31, HEAD `259504e`** (centralisation core + tests Vitest) |
...
24. **Capitalisation financière** — toutes les conversions revenu net d'exploitation (RNE) ↔ valeur ↔ taux de capitalisation (TGA) passent par `@primexpert/core/financial/capitalization.ts` ; aucune formule RNE/TGA locale dans React, PDF ou Functions.
25. **Qualité transaction RPA** — PR #31 ajoute Vitest + workflow `RPA Transaction Test Coverage` : couverture 100 % de `resolveColumnId()` et validation des 7 échéances critiques quand une promesse d'achat (PA) est acceptée.
...
| **QA transaction RPA** | GitHub Actions + Vitest | `resolveColumnId()` + délais PA acceptée | EN REVUE — PR #31 |
...
| Finances (Hub 5 sous-onglets) | ✅ + benchmark global ; RNE/TGA centralisés dans `@primexpert/core/financial/capitalization.ts` (PR #31) |
| Déclaration | ✅ Questionnaire OACIQ |
| Marché | ✅ **Analyse de mise en marché (ACM)** (SSOT finances + TGA GPS) + concurrence territoriale ; TGA cible/ajustements via helpers core |
...
| Promesse | ✅ Cockpit PA (`offre` SSOT) + **assembleur contrat V3.5** (`ContractAssemblerPanel`, export HTML) ; 7 échéances PA acceptée protégées par tests |
...
*Index mis à jour : 2026-06-01 — PR #31 Draft : centralisation RNE/TGA dans le core financier + garde QA transaction RPA (`259504e`).*
