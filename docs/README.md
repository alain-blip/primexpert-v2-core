...
**Cycle technique :** **V3.7 — marché/OER/flywheel + assembleur de mandats V3.5** · **couverture RPA PR #3** (Kanban + délais PA acceptée) · **QA finance/lexique PR #29** (TGA SSOT + RNE vérifié) · architectures sécurisées V3.0 en production
...
3. **Finance** — Document unique `residences/{id}/financial/dataV2` ; contexte `FinancialDataProvider` ; prix/RNE/TGA via `@primexpert/core/financial` (`normalizeTgaPct`, `computeTgaRatioFromRneAndPrice`, `computeCapitalizedValueFromRneAndTga`) + `ResidenceDataContext`.
...
| Finances (Hub 5 sous-onglets) | ✅ + benchmark global + QA finance PR #29 (RNE vérifié, TGA/capitalisation centralisés core) |
