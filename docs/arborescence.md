...
│           │   └── revenusDepensesPreview.ts   # KPIs live + helpers TGA ratio/%/capitalisation (QA PR #29)
...
│   ├── components/
    ...
    │   ├── financialDataService.ts    # Ajustements dépenses ; écrit tauxCapitalisation via helper core TGA
...
**SSOT prix & hints finance (`d232673` + QA PR #29) :** `ResidenceDataProvider` normalise `price` / `prixAnnonce` / `prixDemande` ; `useResidenceFinancialHints()` injecte le prix canonique dans tous les sous-onglets ; core `resolveAdmissibleOpex()` — RNE = RBE − dépenses **déclarées** (pas le normalisé seul) ; TGA/capitalisation passent par `revenusDepensesPreview.ts` (`normalizeTgaPct`, `computeTgaRatioFromRneAndPrice`, `computeCapitalizedValueFromRneAndTga`).
...
| Données financières | `src/context/FinancialDataContext.tsx`, `packages/core/src/financial/`, `revenusDepensesPreview.ts` (helpers TGA QA PR #29) |
