...
│       ├── package.json
│       └── src/
│           ├── index.ts             # Barrel (canonical, valuation, financial, analytics, security, forms…)
│           ├── canonical/           # Champs canoniques & alias
│           ├── financial/           # normalizeFinancialData, capitalization, mergeExtractedFinancials, bilan, ratios, SCHL…
...
│           ├── analytics/              # OER, ratios marché, exports @primexpert/core/analytics
│           │   └── marketMetrics.ts
│           ├── security/               # Vault WORM, journal conformité, profil courtier
│           │   ├── vaultSpecsTypes.ts
│           │   └── brokerProfileCompliance.ts
│           ├── services/
...
│           ├── residence/             # partiesImpliquees, listingSource, inscriptionBrokerageStatus, listingCommission, quebecRegions
...
│           │   ├── listingSource.ts          # Centris vs hors marché ; skip sync
│           │   ├── inscriptionBrokerageStatus.ts
...
│           ├── market/                # Centris comparables, flywheel, semantic anchors, dedupe, closingEngine
│           │   ├── centrisComparableCapRate.ts
│           │   ├── internalMarketFlywheel.ts
│           │   ├── marketPdfSemanticAnchors.ts
│           │   └── marketDeduplication.ts
...
│   ├── scripts/                     # Prebuild vendor @primexpert/core vers Functions
│   │   ├── sync-core-analytics-flywheel.cjs
│   │   ├── sync-core-security.cjs
│   │   ├── sync-core-residence.cjs
│   │   ├── sync-core-documents.cjs
│   │   └── sync-core-market.cjs
│   └── src/
...
│       │   ├── parseMarketDocument.ts       # Parse rapports marché (Vertex + cache MD5)
│       │   ├── injectMarketMacroStats.ts
│       │   ├── marketPdfSlice.ts            # Découpage sémantique avant Vertex
...
│       ├── analytics/               # onTransactionConcludedFlywheel + ingestion market_analytics_raw
│       ├── security/                # onVaultDocumentWrite + LegalComplianceLog SHA-256
│       ├── centris/                 # centrisListingsSyncNightly + sync listings_cache
...
│   ├── components/
    ...
    │   ├── BrokerPhotoComplianceBanner.tsx  # Alerte photo permis courtier > 5 ans
    │   ├── inscriptions/
    │   │   ├── CreateInscriptionForm.tsx    # Création Centris ou hors marché
    │   │   └── InscriptionStatusDropdown.tsx # Statut MLS verrouillé sauf override / hors marché
    │   ├── listings/
    ...
    │   │   ├── market/
    │   │   │   ├── ResidenceAcmValuationPanel.tsx
    │   │   │   ├── AcmTab.tsx
    ...
    │   ├── ACM.tsx, ContentGen.tsx    # ACM Workhub + rédacteur IA Centris / lint OACIQ
    │   ├── Drive/, Softphone/
    ...
    │   ├── inscriptionsService.ts       # CRUD inscriptions hors marché
    │   ├── legalVaultService.ts         # WORM organizations/{orgId}/legal_vault
    │   ├── marketAnalyticsService.ts    # listings_cache + market_analytics_raw
...
Neuf onglets ; coquille bleue institutionnelle (`InstitutionalResidenceTabShell`) sur les onglets concernés et `ResidenceTabErrorBoundary` autour de chaque rendu.
...
| Diffusion | `DiffusionWebTab` | ✅ Publication Web, garde-fous OACIQ, filigrane hors marché |
...
| **Hosting** | `dist/` — SPA, réécriture `**` → `index.html`, `no-cache` explicite sur `/workhub{,/**}` et `/acces-vendeur{,/**}` |
| **URL prod** | https://primexpert-app-v2.web.app |
| **Firestore** | Bases `(default)` + `ai-studio-1214d671-efd2-47da-93b7-425feb92155a` (même rules/indexes) ; WORM `legal_vault`, `listings_cache`, `market_*` multi-tenant |
| **Storage** | `primexpert/{orgId}/contacts/…` ; `primexpert/{brokerId}/properties/{id}/documents/…` ; **`primexpert/{brokerId}/market_documents/…`** |
| **Functions** | `functions/` — Nylas, Espace Documents, **Statistiques du marché**, benchmark global, WORM, flywheel, Centris nightly |
...
| Données financières | `src/context/FinancialDataContext.tsx`, `src/context/ResidenceDataContext.tsx`, `packages/core/src/financial/` (`capitalization.ts`, `resolveCanonicalRne.ts`) |
...
| Inscriptions (cartes, view model, Kanban DnD) | `Listings.tsx`, `inscriptions/CreateInscriptionForm.tsx`, `InscriptionStatusDropdown.tsx`, `ListingInstitutionalCard.tsx`, `listingCardViewModel.ts`, `packages/core/src/residence/listingSource.ts`, `listingCommission.ts` |
...
| Bibliothèque marché (Statistiques du marché) | `MarketLibraryDashboard.tsx`, `marketDocumentsService.ts`, `parseMarketDocument.ts`, `marketPdfSlice.ts`, `injectMarketMacroStats.ts`, `marketDeduplication.ts` |
| Analyse de mise en marché (ACM) | `AcmValuationWorkspace`, `ResidenceAcmValuationPanel`, `TerritorialCentrisCompetitionSection`, `useTerritorialCompetition`, `centrisComparableCapRate.ts`, `residenceAcmBootstrap.ts` |
| Data Flywheel | `onTransactionConcludedTrigger.ts`, `flywheelIngestion.ts`, `internalMarketFlywheel.ts`, `sync-core-analytics-flywheel.cjs` |
| Coffre-fort WORM | `LegalVaultWormPanel.tsx`, `LegalVaultWormLockModal.tsx`, `legalVaultService.ts`, `onVaultDocumentWrite.ts`, `vaultSpecsTypes.ts` |
...
| Espace Documents | `DocumentsDiligenceTab`, `propertyDocumentsService.ts`, `legalVaultService.ts`, `functions/src/documents/`, `functions/src/security/` |
...
| Fonction | Rôle |
|----------|------|
| `marketDocumentParseIA` | Parse Vertex rapports marché — **512 MiB**, **60 s**, slice sémantique `marketPdfSlice.ts` + cache `contentHashMd5` |
| `injectMarketMacroStats` | Injection HITL idempotente avec `orgId` → `market_macro_stats`, `market_analytics_raw`, `marketSnapshots/v1` |
| `getGlobalFinancialBenchmark` | Médianes régionales / portefeuille pour Hub Finance |

### Cloud Functions — sécurité, Centris et flywheel

| Fonction | Rôle |
|----------|------|
| `onVaultDocumentWrite` | Trigger Montréal `organizations/{orgId}/legal_vault/{documentId}` → journal `compliance_logs` SHA-256 |
| `onTransactionConcludedFlywheel` | Trigger `residences/{residenceId}` → transaction anonymisée dans `market_analytics_raw` |
| `centrisListingsSyncNightly` | Sync nocturne Centris vers `listings_cache` ; ignore `listingSource === 'off_market'` |

---

*Dernière mise à jour : 2026-06-01 — PR #10 : QA RNE/TGA centralisés, WORM, flywheel, Centris, hors marché et arborescence docs-only.*
