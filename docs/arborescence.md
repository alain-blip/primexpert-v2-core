...
│           ├── financial/           # normalizeFinancialData, bilan, ratios, SCHL, ACM présentation…
│           │   ├── mergeExtractedFinancials.ts
│           │   └── acmValuationPresentation.ts  # Orchestrateur ACM présentation (prix, TGA, stress)
...
**SSOT prix & hints finance (`d232673` + PR #19) :** `ResidenceDataProvider` normalise `price` / `prixAnnonce` / `prixDemande` ; `useResidenceFinancialHints()` injecte le prix canonique dans tous les sous-onglets ; core `resolveAdmissibleOpex()` — RNE = RBE − dépenses **déclarées** (pas le normalisé seul) ; `acmValuationPresentation.ts` évite les calculs ACM en React.
...
*Dernière mise à jour : 2026-06-01 — PR #19 : QA finance/langage, orchestrateur ACM dans `@primexpert/core/financial`, aucune nouvelle route Functions. Revues PR #33 / PR #44 : diffs documentation-only, aucune nouvelle arborescence applicative.*
