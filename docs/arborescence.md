...
| Données financières | `src/context/FinancialDataContext.tsx`, `src/services/financialDataService.ts`, `packages/core/src/financial/` (`capitalizationMetrics.ts`, `normalizeFinancialData.ts`) |
...
| QA capitalisation / TGA | `capitalizationMetrics.ts`, `valuationEngine.ts`, `marketCapRate.ts`, `lenderCapRate.ts`, `stressTest.ts`, `AcmValuationWorkspace.tsx`, `FinancabiliteTab.tsx` |
...
*Dernière mise à jour : 2026-06-01 — PR #14 : helper `financial/capitalizationMetrics`, QA RNE/TGA et fusion du doublon `financial/` dans l'arborescence.*
