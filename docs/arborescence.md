...
├── .github/
│   └── workflows/
│       └── rpa-transaction-test-coverage.yml  # Vitest — Kanban + délais PA acceptée (PR #31)
...
│           ├── financial/           # normalizeFinancialData, bilan, ratios, SCHL, capitalisation RNE/TGA
│           │   ├── capitalization.ts       # SSOT conversions RNE ↔ valeur ↔ TGA (PR #31)
...
│       ├── analytics/               # Flywheel marché + vendoring capitalization.ts (PR #31)
│       │   ├── flywheelIngestion.ts
│       │   ├── onTransactionConcludedTrigger.ts
│       │   └── _vendored/capitalization.ts
│       ├── centris/                 # Synchronisation Centris / RESO et statuts inscription
│       │   ├── centrisListingsSyncNightly.ts
│       │   ├── syncCentrisListingsNightly.ts
│       │   └── _vendored/
│       ├── documents/
...
│       ├── security/                # WORM légal — onVaultDocumentWrite + journal SHA-256
...
│   ├── check-resolveColumnId-coverage.mjs  # Gate Vitest 100 % sur resolveColumnId()
...
├── index.html
├── package.json
├── vitest.config.ts                 # Tests ciblés RPA / couverture transaction (PR #31)
...
| Capitalisation financière RNE/TGA | `packages/core/src/financial/capitalization.ts`, `functions/src/analytics/_vendored/capitalization.ts`, `src/services/financialDataService.ts` |
...
| QA transaction RPA | `.github/workflows/rpa-transaction-test-coverage.yml`, `vitest.config.ts`, `src/config/__tests__/resolveColumnId.test.ts`, `packages/core/src/transaction/__tests__/paAccepteeCriticalDeadlines.test.ts` |
...
| `onTransactionConcludedFlywheel` | Ingestion anonyme de transactions conclues → `market_analytics_raw` via helpers RNE/TGA core |

### Cloud Functions — Centris / sécurité

| Fonction | Rôle |
|----------|------|
| `centrisListingsSyncNightly` / `syncCentrisListingsNightly` | Synchronisation nocturne Centris / RESO, statut inscription et exclusion des fiches hors marché |
| `onVaultDocumentWrite` | Journal WORM légal `legal_vault` / `compliance_logs` (Montréal), chaînage SHA-256 |
...
*Dernière mise à jour : 2026-06-01 — PR #31 Draft : centralisation RNE/TGA (`capitalization.ts`) + workflow QA transaction RPA (`259504e`).*
