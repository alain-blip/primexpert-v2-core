...
const REVENUE_TOTAL_PRIORITY_PATTERN =
  /^(produits?\s+totaux?|total\s+des\s+produits|produits?\s+d.?exploitation|revenus?\s+totaux?|chiffre\s+d.?affaires(\s+total)?|total\s+des\s+revenus|revenu\s+brut\s+effectif|effective\s+gross\s+income|gross\s+revenue)/i;
